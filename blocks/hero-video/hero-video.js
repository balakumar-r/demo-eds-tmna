...
function createPlayIcon() {
    return `
    <svg class="hero-video-play-icon" viewBox="0 0 92 91" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
...
export default function decorate(block) {
    ...
    if (!videoUrl || !videoId) {
        block.innerHTML = '';
        block.classList.add('hero-video-error');
    ...
    <div class="hero-video-inner">
      <div class="hero-video-poster" style="background-image: url('${escapeAttribute(poster)}')">
        <button class="hero-video-play" type="button" aria-label="Play video">
          ${createPlayIcon()}
        </button>
      </div>

      <div class="hero-video-player" aria-hidden="true"></div>
    </div>
  `;

    const posterEl = block.querySelector('.hero-video-poster');
    const playBtn = block.querySelector('.hero-video-play');
    const player = block.querySelector('.hero-video-player');
    ...
    const sectionWrapper = block.closest('.hero-video-wrapper');
